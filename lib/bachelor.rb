...
winner_array = []
  ...
  data.each do |season, data_array|
    data_array.each do |h|
        if h["status"] == "Winner" && season_input == season
          winner_str = h["name"]
          winner_array = winner_str.split
...
def get_contestant_name(data, occupation_input)

  data.each do |season, data_array|
    data_array.each do |h|
      #binding.pry
      if h["occupation"] == occupation_input
        #binding.pry
        return h["name"]
      end
    end
...
age_array = data[season].map do |data_hash|
              data_hash["age"].to_i
            end
